...
const OrderDetailsRow = lazy(() =>
  import("./../../../../components/OrderDetailsRow")
);
...
const PageSlugOrder = () => {
  ...
  const address = useLocation(
    orderDetail?.cityCode,
    orderDetail?.districtCode,
    orderDetail?.wardCode
  );
  // console.log(address)
  ...
  const fetchOrderList = useCallback(async () => {
    ...
    if (masterCapacity?.length > 0) {
      cartItemsInfo.map((e) => {
        ...
        const capacity = masterCapacity?.find(
          (cap) => cap.id === e.capacityId
        )?.name;
  ...
  }, [info?.id, masterCapacity, masterUnit, slugOrder]);
