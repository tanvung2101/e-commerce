"use client"
import { configPageApisClient } from '@/apis/configPageApis';
...
const PageContact =  () => {
  ...
  const onSubmit = (data) => {
    ...
    async function getData(){
        const result = await configPageApisClient.getListConfigPageContact()
        const contact = result.find((ct) => ct.type === CONTACT_PAGE.CONTRACT);
        setContact(contact)
    }
    useEffect(() => {
        getData()
